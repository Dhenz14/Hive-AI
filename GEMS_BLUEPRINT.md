...
|-----------|-----------|-------------------|-------|
| Padding-Free Sequence Packing | **Hive-AI** | Next Layer 3 training event. Enable only after validating STM boundary handling (see note). | Unsloth December 2025. 3x faster training + 30% VRAM reduction. Eliminates padding tokens by concatenating samples into dense blocks. **Integration risk**: STM computes per-token PPL on base model — if packing crosses sequence boundaries, PPL values become meaningless at boundary tokens and masking will be wrong. Must verify Unsloth's packing preserves position IDs and attention masks per original sequence. SDFT is less sensitive (KL operates on output distribution, not position-specific). If boundary handling is correct, this is the highest-value Unsloth integration for our Layer 3 pipeline: 30% VRAM = more headroom for rank/replay ratio on 16GB GPU. |
